...
export const LoginForm = ({ onLogin }) => {
  ...
  const validateRoll = (val) => {
    if (!/^25\d{4}$/.test(val)) return false; // must start with 25 + 4 digits
    const num = parseInt(val.slice(2), 10);
  ...
  return (
    <div className="min-h-screen bg-gradient-background flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-6 relative z-10">
        {/* Header - centered logo + glow + titles */}
        <motion.div
        ...
        >
          ...
          >
            ...
            <motion.div
              ...
              transition={{ duration: 3.5, times: [0, 0.7, 1], repeat: Infinity }}
            ...
            <img
              src="/smslogo.png"
              alt="SciMathSoc Logo"
              className="w-20 h-20 object-cover rounded-full shadow-xl relative z-10"
              style={{ display: "block" }}
            />
          </div>
          ...
          <motion.div
            className="flex items-center justify-center space-x-4 text-sm text-muted-foreground mt-2"
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <Input
                id="rollNumber"
                type="text"
                placeholder="e.g., 251234"
                value={rollNumber}
                onChange={(e) => onRollChange(e.target.value)}
                className={`bg-input border-primary/20 text-foreground placeholder:text-muted-foreground focus:ring-primary/20 focus:border-primary py-3 text-lg transition-all ${
                  rollNumber.length > 0 && !rollValid ? "border-red-400 animate-shake" : ""
                }`}
                disabled={isLoading}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onKeyPress={(e) => e.key === "Enter" && !isLoading && handleLogin()}
                className={`bg-input border-primary/20 text-foreground placeholder:text-muted-foreground focus:ring-primary/20 focus:border-primary py-3 text-lg transition-all ${
                  name.length > 0 && !nameValid ? "border-red-400 animate-shake" : ""
                }`}
                disabled={isLoading}
              />
            </div>
            ...
            >
              <Button
                onClick={handleLogin}
                disabled={
                  !rollNumber.trim() || !name.trim() || isLoading || !rollValid || !nameValid
                }
                className="w-full bg-gradient-primary hover:shadow-primary text-white font-medium h-14 text-lg transition-all duration-300"
              >
                {isLoading ? (
            ...
            <div className="text-center pt-2">
              <p className="text-xs text-muted-foreground">Bold Minds. Sharp Ideas. One Society.</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-card border-primary/20 shadow-card backdrop-blur-lg">
          <CardContent className="p-6">
            ...
            <div className="mt-4 pt-3 border-t border-primary/20 text-center">
              <p className="text-xs text-muted-foreground">🎯 8 Questions • ⚡ Speed Scoring • 📈 Live Rankings</p>
            </div>
          </CardContent>
        </Card>
      </div>
